...
// Only admins can use this endpoint — staff accounts are blocked.
...
function is_strong_password(string $password): bool {
    ...
    if (!preg_match('/[!@#$%^&*()\-_=+\[\]{};:\'",.<>\/?\\\\|]/', $password)) return false; // special char
    return true;
}

// -- GET: return all accounts (admin only) --
...
// -- Change password --
// Both admin and staff can change their OWN password
// Only admins can change SOMEONE ELSE'S password
if ($action === 'change_password') {
    $logged_in    = validate_token($conn);
    ...
    if ($logged_in['role'] !== 'admin' && $logged_in['id'] !== $id) {
        respond(false, null, 'Forbidden: you can only change your own password', 403);
    }
    ...
    db_execute(
        ...
        'ssss',
        [$logged_in['username'], 'change_password', $rows[0]['username'], 'Password updated']
    );

    respond(true, null, 'Password updated');
}

// -- Add and delete are admin-only --
$logged_in = require_admin($conn);
...
if ($action === 'add') {
    ...
    db_execute(
        ...
        'INSERT INTO logs (actor, action, target, detail) VALUES (?, ?, ?, ?)',
        'ssss',
        [$logged_in['username'], 'add_account', $username, "Role: $role"]
    );
...
if ($action === 'delete') {
    ...
    db_execute(
        ...
        'INSERT INTO logs (actor, action, target, detail) VALUES (?, ?, ?, ?)',
        'ssss',
        [$logged_in['username'], 'delete_account', $rows[0]['username'], "Role was: {$rows[0]['role']}"]
    );
